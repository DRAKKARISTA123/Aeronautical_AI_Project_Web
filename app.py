...
with tab2:
    ...
    st.markdown("*Data Source:* **Abbott & von Doenhoff / NACA TR-824 Experimental Polars ($Re \approx 3 \times 10^6$)**")
    st.write("This tab evaluates aerodynamic performance entirely via piecewise linear interpolation over official wind-tunnel coordinate tables, eliminating synthetic closed-form approximations.")
    ...
    ax.plot(alphas_range, cls_range, label="NACA 2412 Wind-Tunnel Interpolation ($C_L$)", color="blue", linewidth=2)
    ...
    st.write("To verify that this model is driven by real wind-tunnel coordinates rather than hidden equations, inspect the underlying empirical dataset and active lookup calculation below:")
    
    with st.expander("📂 Click to view the raw NACA 2412 experimental data arrays used by `np.interp()`"):
